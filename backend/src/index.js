...
(async () => {
  try {
    ...
    // Ensure customers.id has auto-increment configured (IDENTITY or sequence)
    try {
      const [custTableExists] = await db.sequelize.query(
        "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers');"
      );
      if (custTableExists[0].exists) {
        const [idInfo] = await db.sequelize.query(
          "SELECT is_identity, column_default FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'id';"
        );
        const isIdentity = idInfo && idInfo[0] && idInfo[0].is_identity === 'YES';
        const hasSequence = idInfo && idInfo[0] && (idInfo[0].column_default || '').includes('nextval');
        if (!isIdentity && !hasSequence) {
          await db.sequelize.query('ALTER TABLE customers ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;');
          console.log('Configured customers.id as IDENTITY for auto-increment.');
        } else {
          console.log('customers.id already has auto-increment configured');
        }
      }
    } catch (e) {
      console.warn('Warning: Could not verify/configure customers.id identity:', e.message || e);
    }
